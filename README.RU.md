...
|------|--------|------------|
| Typed ORM | ✅ Стабильно | Полная типобезопасность на уровне моделей, менеджеров, кверисетов |
| Typed Manager | ✅ Стабильно | Generic-менеджер со схемо-зависимыми операциями |
| Typed QuerySet | ✅ Стабильно | Чейнебл билдер запросов с полной типизацией |
| Schema Registry | ✅ Стабильно | Централизованное обнаружение схем моделей |
| Pydantic Bridge | ✅ Стабильно | Двусторонняя синхронизация Django-моделей и Pydantic-схем |
| Unified Validation | ✅ Стабильно | Одна схема — Forms, DRF, FastAPI, ORM, Celery |
| Cache Abstraction | ✅ Стабильно | Подключаемый кеш-слой с детерминированными ключами |
| Memory / Redis / Django / Null Cache Backends | ✅ Стабильно | Все четыре бэкенда протестированы в проде |
| Metrics | ✅ Стабильно | Встроенные счётчики производительности |
| OTEL Tracing | ✅ Стабильно | OpenTelemetry-спаны на `Model.save()`, zero config |
| Distributed Task Engine | ✅ Стабильно | Асинхронная очередь задач с воркерами |
| Retry Policy | ✅ Стабильно | Экспоненциальный бэкоф с джиттером |
| Delayed Tasks | ✅ Стабильно | Отложенный запуск задач |
| Priority Queue | ✅ Стабильно | Приоритизация задач из коробки |
| Stable Public API (базовая версия) | ✅ Стабильно | Core-экспорты заморожены, semver |
| Query Planner (PoC) | 🧪 Экспериментально | Анализ запросов и генерация `QueryPlan` — ранняя стадия |
| Django 6 Compatibility (базовая) | 🧪 Экспериментально | Базовая совместимость проверена, полный аудит в процессе |

---

## 🚧 В разработке

| Фича | Статус | ETA | Блокер |
|------|--------|-----|--------|
| Redis Infrastructure | 🚧 В работе | Q3 2026 | Единый Redis-клиент, распределённые блокировки, Pub/Sub, rate limiter, health checks |
| Read Replica Router | 🚧 В работе | Q3 2026 | Автоматическое разделение чтения/записи с учётом лага реплики |
| Full Query Planner | 📋 В планах | Q4 2026 | Анализ запросов → оптимизированный `QueryPlan` с оценкой стоимости |
| Prefetch Optimizer | 📋 В планах | Q4 2026 | Авто `select_related` / `prefetch_related` / `only` / `defer` |
| Stable Public API (заморозка) | 📋 В планах | Q4 2026 | Полная заморозка экспортов, гарантия обратной совместимости |
| Django 6 Audit | 📋 В планах | Q4 2026 | Полная проверка совместимости без изменения публичного API |
| Full Observability | 📋 В планах | Q4 2026 | Trace + Metrics + Logs + Correlation IDs |
| Production Readiness | 📋 В планах | Q1 2027 | Health checks, стресс-тесты, бенчмарки, матрица совместимости |
...
**Текущая фаза:** Core-платформа стабильна — слой инфраструктуры и оптимизации в активной разработке.
